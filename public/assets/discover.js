...
(function () {
  ...
  // 🔴 **거리와 무관하게 텍스트 하나다** (사용자 결정 2026-09-22 · SPEC §CH3 · DECISIONS (8)).
  // 예전엔 중·장거리 직항만 청록 배지(`✈ 직항`)였다 — 「근거리 직항은 당연하고 장거리 직항은 소식이다」는
  // 규칙이 있었고 그 자체는 틀리지 않았다. 그런데 사용자가 실사용에서 물었다:
  // 「어떤 직항은 아이콘 배지고 어떤 건 두 글자던데 의도된 거냐」
  //
  // **규칙을 아는 사람에게만 규칙이었다.** 화면에 「이건 거리가 멀어서 배지다」라고 적혀 있지 않으니
  // 읽는 쪽에는 그냥 **들쭉날쭉**으로 보인다. 같은 말(`직항`)을 두 모양으로 그리면
  // 사용자는 **두 모양이 다른 뜻이라고 읽는다** — 없는 구분을 만들어 낸 셈이다.
  function transportHTML(dl) {
    return dl.transfers > 0
      ? '<span class="trx">경유 ' + dl.transfers + "회</span>"
      : '<span class="trx">직항</span>';
  }
  ...
  function detailHTML(c) {
    ...
    return '<button type="button" class="hc-x" aria-label="상세 닫기">×</button>' +
      ...
      // 57~268px 아래였다. 여기는 「이 가격이 평소보다 싼가」를 막 읽은 자리라 **「그럼 이 노선은 평소에 얼마지」가
      // 바로 다음 질문**이고, 스크롤 없이 닿는다.
      //
      ...
      // 🔴 **`(광고)` 설명은 예약처 목록 _위_에 둔다** — 목록의 캡션이다 (DESIGN.md §접근성, 2026-09-20).
      // 아래에 두면 **뜻을 모른 채 누를 수 있다**: 카드 높이는 지도(핀 y)가 정해서 데스크톱에선 목록 아래가
      // 카드 안 스크롤 밑으로 내려간다(실측 1440×900·1366×768 둘 다). 「스크롤 없이 보이게」는 지킬 수 없는
